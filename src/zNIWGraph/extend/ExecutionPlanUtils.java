...
public class ExecutionPlanUtils {
    ...
    public static List<List<Integer>> compute_matching_order(QueryGraph query, PartitionedEdges edges) {
        ...
        queryEdges.sort(Comparator.comparingInt(edge -> {
            ...
            int candidatesNum = edges.get_edge_label_frequency(labels);
            return candidatesNum;
        }));

        printCandidates(query_graph, queryEdges, edges);
    ...
    private static void printCandidates(DynamicHyperGraph query_graph, List<List<Integer>> queryEdges, PartitionedEdges edges) {
        int sum = 0;
        for (List<Integer> edge : queryEdges) {
            List<Integer> labels = query_graph.getEdgeLabels(edge);
            int num = edges.get_edge_label_frequency(labels);
            sum += num;
